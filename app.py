...
# ── Sidebar ───────────────────────────────────────────────────────────────────
with st.sidebar:
    st.markdown("## 📊 Portal de Ventas")
    archivo = st.file_uploader("📁 Excel de ventas (ERP)", type=["xlsx","xls"])
    archivo_coords = st.file_uploader("📍 Excel de coordenadas (opcional)", type=["xlsx","xls"])
    if archivo:
        rol = st.radio("Ingresar como:", ["Vendedor","Gerencia"])
    else:
        rol = None
        st.info("Subí el archivo Excel exportado del ERP para comenzar.")

if not archivo:
    st.title("📊 Portal de Ventas")
    st.markdown("""
    ### Bienvenido
    - 📋 Todos tus clientes asignados, compren o no
    - ⚠️ Clientes inactivos o sin compras en el período
    - 📅 Facturación mensual comparativa
    - 🗺️ Mapa de clientes por estado o vendedor

    **👈 Subí el Excel en el panel izquierdo para comenzar.**
    """)
    st.stop()
